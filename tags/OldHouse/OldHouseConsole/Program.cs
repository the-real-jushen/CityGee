...
namespace OldHouseConsole
{
    class Program
    {
        ...
        {
            string remoteSrv = @"mongodb://115.156.252.231:27017";
            ...
            loadExcel(@"G:\project\Parallel\OldHouse\code\OldHouse\TeastDataSet\test1\data.xls", int.Parse(args[0]), int.Parse(args[1]));
        ...
        static void loadExcel(string file,int startrow, int endrow)
        {
            ...
            for(int row=startrow;row<=endrow;row++)
            {
                ...
                try
                {
                    photos = getPhotos(path).ToList<string>();
                }
                catch 
                {
        ...
        static IEnumerable<string> getPhotos(string path)
        {
            foreach (var file in Directory.GetFiles(path))
            {
                yield return file.Substring(file.LastIndexOf(@"\") + 1, file.Length - file.LastIndexOf(@"\")-1);
            }
        }
